...
As DLLs nativas da Suprema (`UFScanner.dll`, `UFMatcher.dll`) são **proprietárias** e **não são redistribuídas** neste repositório — apenas o procedimento de patch e o código do wrapper são abertos. Usuários finais devem obter as DLLs do SDK de seus próprios backups/instalações OEM. O patch é fornecido para interoperabilidade com hardware de sua propriedade.

---

## Contribuição

PRs são bem-vindos. Áreas de alto valor: a bridge, o CLI, experimentos Linux/macOS e o driver WBF.

Construído a partir das notas de campo de uma missão real de resgate (2026-08-21): diagnóstico do driver → arqueologia do SDK (Internet Archive, mirrors no GitHub) → patch binário → app nativo funcionando em um dia. História completa em [`docs/reverse-engineering.md`](docs/reverse-engineering.md).
